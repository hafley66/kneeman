...
//!
//! Exercises `world::store` (sqlite), so it only compiles under `--features storage`.
#![cfg(feature = "storage")]

use smash_core::world::bridge::{Bridge, PlatParams, SimGeo};
use smash_core::world::fold::fold;
use smash_core::world::store::{SqliteStore, WorldStore};
use smash_core::world::{BuildVersion, Seed, StageId};
